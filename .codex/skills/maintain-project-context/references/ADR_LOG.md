...
- REF-08 aligned all Go code with the project code style: `_`-prefixed unexported package constants and variables, exported `config.ValidationError`, blank lines after checks and before trailing returns, separate constant groups for unrelated values, and lines within the 99-character soft limit outside struct tags.
